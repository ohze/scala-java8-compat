...
object WrapFnGen {
  ...
  /** @param sig - ex: `BiConsumer[T,U]: accept(T,U): Unit` | `DoubleToIntFunction: applyAsInt(Double): Int` */
  case class Jfn(sig: String) {
    val Array(
    iface, // interface name included type params, ex: BiConsumer[T,U] | DoubleToIntFunction
    _method, // Temp val, ex: accept(T,U) | applyAsInt(Double)
    rType // java function return type, ex: Unit | Int
    ) = sig.split(':').map(_.trim)

    // interface name and java interface's type args, ex: (`BiConsumer`, `[T,U]`) | (`DoubleToIntFunction`, ``)
    ...
    // java.util.function.BiConsumer[T,U] | java.util.function.DoubleToIntFunction
    val javaFn = s"java.util.function.$iface"

    // scala.Function2[T, U, Unit] | scala.Function1[Double, Int]
    ...
    /** @return `implicit def enrichAsJavaXX` */
